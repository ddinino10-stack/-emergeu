...
function Romeo({ user, onBack }) {
  const [messages, setMessages] = useState([
    {
      role: 'ai',
      text: "Hey! I'm Romeo, your personal AI fitness coach 💪🏼 I'm here 24/7 to help you with workout plans, nutrition advice, meal plans and anything fitness related. What are we working on today?"
    }
  ]);
  ...
  const sendMessage = async () => {
    ...
    if (!isPro && isPremiumRequest(userMessage)) {
      setTimeout(() => {
        setMessages([...newMessages, {
          role: 'ai',
          text: "I'd love to create a personalised plan for you! 🔥 This is a Romeo Pro feature — upgrade to get unlimited meal plans, workout programmes and much more for just £9.99/month 💪🏼"
        }]);
    ...
    try {
      ...
      const response = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({
          ...
          max_tokens: 1000,
          system: `You are Romeo, a world class personal fitness coach and nutritionist. You work for EmergeU, a premium fitness platform.
    ...
    <Subscribe
      ...
      onSuccess={() => {
        ...
        setMessages(prev => [...prev, {
          role: 'ai',
          text: "Welcome to Romeo Pro! 🎉🔥 You now have full access to personalised meal plans, workout programmes and everything else. Now, let's get back to building that plan for you — what would you like first?"
        }]);
      }}
    />
  );
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ flex: 1 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            <h3 style={{ color: 'white', margin: 0, fontSize: '18px' }}>Romeo</h3>
            {isPro && (
              <span style={{
      ...
      <div style={{
        ...
        <input
          type="text"
          placeholder={isPro ? "Ask Romeo anything..." : "Ask Romeo a question (meal plans require Pro)..."}
          value={input}
